  ...
  try {
    ...
    for (let i = 7; i >= 0; i--) {
      ...
      for (const goal of dailyGoals) {
        ...
        targets.push({
          ...
          userId: DEFAULT_USER_ID,
          goalId: goal._id,
          completedAt: completed ? new Date(targetDate.getTime() + Math.random() * 24 * 60 * 60 * 1000) : null
        });
      }
    }
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const dayTargets = targets.filter(t => 
        t.date.toDateString() === entryDate.toDateString()
      );
      ...
      const mood = completed / total >= 0.8 ? 'excellent' : 
                   completed / total >= 0.6 ? 'good' : 
                   completed / total >= 0.3 ? 'okay' : 'difficult';
      ...
      dayEntries.push({
        ...
        targets: dayTargets.map(t => ({
          ...
          category: createdGoals.find(g => g._id.equals(t.goalId))?.category || 'General'
        }))
      });
    }
    ...
    console.log(`🎖️ User Achievements: ${await UserAchievement.countDocuments()}`);
